...
class Assignment_Map { 
	 ...
	 * UPDATE_ASMT_MAP
	 *
	 * 	Descrip: Update the assignment map from the database's options table. Does this actually work? 
	...
	public function update_asmt_map( $post ){
		...
		$asmt_map_data = $this->get_asmt_map_data(); //This is a flattened array. 
		
		//I Flattened the array to update the data. 
		foreach( $post as $p_key => $p_val ){
			if( array_key_exists( $p_key, $asmt_map_data ) )
					$asmt_map_data[$p_key]->post_id = $p_val;	//We're just updating the Post ID?  	
		}
		
		//Then I reinserted the data back into the expanded array. Necessary? 
	...
	*/
	 ...
	 * 	asmt_map_manager
	 *
	 * 	Descrip: Assignment Map Manager
	 *	
	 *
	 * @since 0.1
	 **/
	 ...
	 * 	get_asmt_map_ids
	 *
	 * 	Descrip: This was ridiculous how many times I went diving through this array. 
	 * 	This is to get Map IDs.
	 *
	 *	
	 * @since 0.1
	 **/
	 ...
	 * 	get_asmt_map_data
	 *
	 * 	Descrip: This takes the data out of the assignment map and compresses it into a one-layer array. 
	 *	The keys of this new array are the unique assignment keys (same thing could be accomplished with
	 *	the post_id. The value is the assignment object. 
	 *
	 *	@return Array
	 *
	 * @since 0.1
	 ...
	 * set_asmt_map_asmts
	 *
	 * 	Descrip: Prepares the assignment map array to be reinserted into the database.
	 *	This is being called by the update_asmt_map method. 
	 *
	 *
	 * @since 0.1
	 **/
	...
	public function asmt_map_form(){
		...
		$asmt_post_args = array(
			'numberposts' =>  -1, //all available posts
			'post_type' =>  'course',
			'meta_key' =>  'course_type',
			'meta_value' =>  1 //for assignments course_type metadata
		);
	...
	public function unmapped_asmts(){
		...
		//This pull all assignment tagged course CPTs 
		$asmt_post_args = array(
			'numberposts' =>  -1,
			'post_type' =>  'course',
			'meta_key' =>  'course_type',
		...
		//Filter out all posts except those that are unmapped. Posts were already loaded a few lines above. 
	 ...
	 *
	 * Maybe this was being used externally? Not used internally (privately). 
	 *
	 * @since 0.1
	 */
