 ...
 *	
 *	This was originally the STUDENT class. Needed name for other functionality, so all this original functionality is being stored here. 
 *	
...
class Student_Processor { //Not sure we need the WP_User class, but it could be helpful.
	 ...
	 * 
	 * called: 
	 *  - editor.class.php, Editor::prepare_import_csv, approx line 1204.
	 * 
	 **/
	...
	//This processes the data imported from Paypal transactions on the Students Import screen. 
		...
		if( empty( $sid ) ){//if Student ID is empty, this is a new student. we don't need to define the student posts if it's an update. 
			...
			$pArr = array(9,15,18,20,25,34,36,50);
	...
	//This processes the data imported from Paypal transactions on the Students Import screen. 
		...
		if( isset( $ppArr['address_name'] ) ){
			$sName = $ppArr['address_name'] = $this->clean_paypal_names( $ppArr['address_name'] );
		}else{
			$sName = $ppArr['address_name'] = $ppArr['first_name'].' '.$ppArr['last_name'];
		}
...
/* 		if( empty( $sPost['student_phone'] ) && !empty( $ppArr['Contact_Phone_Number'] ) ){
		 ...
		 */
	 	...
	 	if( empty( $sid ) ){//if Student ID is empty, this is a new student. we don't need to define the student posts if it's an update. 
			...
			$pArr = array(/* 9,15, */18,20,34,36,50);
			$fArr = array(/* 100, */25,180,200,380,400);
