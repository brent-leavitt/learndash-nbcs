...
class Transaction{
	...
	public function __construct( $id ) {
		if( !empty( $id ) ){
			...
			//echo "<br>Transaction::__construct(), this->tPost is {$this->tPost['student_id']}.";
		} else {
	 ...
	 *
	 * Called:
	 *  -editor.class.php, Editor::prepare_import_csv, approx line 1189.
	 *  
	 * 
	 **/
	...
	//	echo "the SID is $sid, The ppID is: $ppID <br>";
		...
		$trans_id = $wpdb->get_var("SELECT transaction_id FROM transactions WHERE pp_txn_id = '$ppID' AND student_id = '$sid' LIMIT 1");
	 ...
	 *
	 * Called in:
	 *  -editor.class.php, Editor::process_import_csv() approx line 1373
	 *	-ipn_relay.php, process_transaction() approx line 231
	 *  
	 * SHOULD ALSO BE INCLUDED IN: 
	 *	-editor.class.php, Editor:: load_transaction_editor(), approx line 578 Call twice...
	 *
	 *
	 **/
	public function process_transaction($nu = 0, $override = 0, $display = 0) {
		...
		$trans_id = $wpdb->get_var("SELECT transaction_id FROM transactions WHERE pp_txn_id = '$ppID' LIMIT 1");
		...
		if( !empty( $trans_id ) ){ //transaction already exists. 
			if(!$override){
				...
				if($display) echo "<p>This transaction already exists for $stud_link. TransID is: $trans_id.</p> <br />";
				...
				$db_updated = $wpdb->update( 'transactions', $tPost, $tIDarr, $transFormat );
			...
			//echo "<br>Transaction::process_transaction(), line 219. Transaction doesn't exist. We need to add it! Value of $tPost is:";
			//print_pre($tPost);
			...
			if( ( $wpdb->insert( 'transactions', $tPost, $transFormat ) ) === FALSE ){
				
				$batch_report['errors'][] = 'Transaction::process_tranasction(), line229. Failed to insert new transaction.';
				...
				if($transaction_id != null) {
					...
					$batch_report['errors'][] = 'Transaction::process_tranasction(), line 273.No transaction ID has been returned.';
				}
			}
		...
		if( $processed ){
			...
			if($display) echo "<p>Transaction #$trans_id for $stud_link has been processed.</p> <br />";
		...
		//These need to be grouped into a new function called: update_trxn_course_info... come back to this. As it should apply to all transaction type updates: batch processed, manual additions, auto-IPM updates. 
			...
			$batch_report['errors'][] = 'Transaction::process_tranasction(), line 299.The transaction failed to be processed';
		}
	 ...
	 *
	 * 
	 * called in editor.class.php Editor::process_import_csv, approx line 1395.
 	 *
	 **/
...
/* 			case 100:
	 ...
	 *
	 *
	 * Called in: Transaction::process_transaction
	 *
	 ...
	 *
	 *
	 * Called in Tranaction::update_trxn_course_info(), line 426.
	 *
	 **/
	 ...
	 **/
  	...
  	public function set_course_access( array $payArr, $nu = 0, $display = 0){ 
		...
		if( ($nu == 1) && ($p_rate == 'f') ){
			...
			$cAccess = 3; //Limited Access, Level 1
			...
			if($display) echo "Transaction::set_course_access() cAccVal is $cAccVal. ";
	 ...
	 **/
  	public function checkPayRcvd(){
		...
		//pull transactions
		$transArr = array();
		$transQ = "SELECT * FROM transactions WHERE student_id = $sid AND trans_type = 'payment'";
	...
	public function email_receipt($emlWho, $emlWhat, $emlWhy){
		...
		$rcptText = "<hr style='border: 1px solid #e5e5e5; height: 1px;' />
<p>
<b>New Beginnings Transaction ID:</b> $nbti<br />
<b>Date:</b> $tDate <br />
<b>From:</b> ($tFrom, $tEmail) <br />		
<b>Amount:</b> $tAmount <br />
<b>Description:</b> $tDes <br />
<b>Paypal Transaction ID:</b> $tPpTxnId <br />
<b>Payment Status:</b> $tPayStat <br />
</p><br />";
		
		//$emlWhy is the type of email to send.
		switch($emlWhy){
			case 'new_student':
				$emlSubject = 'Getting Started';	
				$txnID = $ewt['txn_id'];
				$regURL = home_url()."/complete-registration?tx_id=".$txnID ;		
				
				$emlMsg = "
<p>Thank you for registering with New Beginnings Postpartum Doula 
Training! Please complete the registration process 
by clicking on the following link:</p> 
<p>
<a style='color:#9EBB09;' href='$regURL' target='_blank'>$regURL</a></p>
<p><small>
(NOTE: You may have already completed this step if 
you were automatically redirected to the registration 
form after your payment was received.)</small></p>

<p>After completing the registration, a username and 
...
$rcptText ";				
			...
			case 'admin':
				$emlSubject = 'Admin Notification - Payment Receipt';
				$emlMsg = "
A payment has been processed on 
the wordpress-87259-1435049.cloudwaysapps.com website. 
$rcptText ";
				break;			
				
			case 'payment':
			default:
				$emlSubject = 'Payment Receipt';
				$emlMsg = "
Thank you for your payment to New Beginnings Postpartum Doula
Training. A copy of your payment receipt is included
below for your records.
$rcptText ";
				break;
		}
/******///Need to check $emlWho; 
		
		$nbEmailConfirm = new message();
		$nbEmailConfirm->recipient = (strcmp( $emlWhy, 'admin') == 0)? 6 :$this->tPost['student_id']; //Make sure this is being sent to admin if is ADMIN. 
		$nbEmailConfirm->subject = $emlSubject;
		$nbEmailConfirm->content = $emlMsg;
			
			
			
		//echo " Did we get to line 865 in NB Trans class?";
		
		$mlRslts = $nbEmailConfirm->send_email();	
		
		//echo " Results of mlRslts is $mlRslts";
